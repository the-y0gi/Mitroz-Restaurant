...
const Navbar = () => {
  ...
  // Scroll detection for compact mode and blur intensity
  useEffect(() => {
    const handleScroll = () => {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            ...
            <a href="/" className="flex items-center gap-3 group relative z-50">
              <div className="relative w-10 h-10 overflow-hidden rounded-xl shadow-lg group-hover:shadow-orange-500/40 transition-all duration-300 ring-1 ring-black/5">
                <img
                  src={img}
                  alt="Mitroz"
              ...
              <span className={`text-xl font-bold tracking-tight transition-colors duration-300 ${isScrolled ? "text-slate-900" : "text-slate-800"}`}>
            ...
            <div className="hidden md:flex items-center bg-white/50 backdrop-blur-md px-2 py-1.5 rounded-full border border-white/20 shadow-sm ring-1 ring-black/5">
              <ul className="flex space-x-1">
                {navLinks.map((link) => (
                  <li key={link.name}>
                    <a
                      href={link.href}
                      className="relative px-4 py-2 text-sm font-medium text-slate-600 hover:text-orange-600 rounded-full hover:bg-white/80 transition-all duration-200 block group"
                    >
                      {link.name}
                    </a>
                  </li>
            ...
            {/* 3. RIGHT SIDE ACTIONS (Profile + Mobile Toggle) */}
            <div className="flex items-center gap-4 relative z-50">
              {/* Desktop CTA */}
              <a
                href="/my-profile"
                className="hidden md:flex items-center gap-2 bg-slate-900 hover:bg-orange-600 text-white px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 shadow-lg shadow-slate-900/20 hover:shadow-orange-500/30 hover:-translate-y-0.5 group"
              >
              ...
              <button
                ...
                className="md:hidden p-2 text-slate-600 hover:bg-slate-100 rounded-full transition-colors active:scale-95"
              >
